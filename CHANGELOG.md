...
- New `from_git` and `from_url` methods of `CHAT` for remote data sources.
- Format conversion between CHAT and the following:
...
- The `to_chat` method of `CHAT` has been renamed `to_files` for clarity.
...
### Changed
- Switched to Rustling for handling CHAT data.
